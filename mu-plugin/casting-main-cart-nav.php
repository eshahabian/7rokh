 ...
 * Description: آیکون سبد خرید کنار شبکه‌های اجتماعی هدر + شمارنده
 * Version: 1.1
 *
...
';
    ...
    wp_register_style('casting-main-cart-nav', false, [], '1.1');
    ...
    $js = '(function(){'
        ...
        . 'h=(h||"").toLowerCase();'
        . 'return h.indexOf("facebook.com")!==-1||h.indexOf("fb.com")!==-1'
        . '||h.indexOf("twitter.com")!==-1||h.indexOf("x.com")!==-1'
        . '||h.indexOf("instagram.com")!==-1||h.indexOf("t.me")!==-1||h.indexOf("telegram")!==-1'
        . '||h.indexOf("linkedin.com")!==-1||h.indexOf("youtube.com")!==-1||h.indexOf("aparat.com")!==-1;'
        . '}'
        . 'function findAnchor(){'
        . 'var scopes=["header",".jeg_header",".td-header-wrap",".tdb-header-template",".site-header",".main-header","#header",".top-bar",".topbar",".header-top"];'
        . 'var i,j,scope,links,a;'
        . 'for(i=0;i<scopes.length;i++){'
        . 'scope=document.querySelector(scopes[i]);'
        . 'if(!scope) continue;'
        . 'links=qsAll("a[href]",scope);'
        . 'for(j=0;j<links.length;j++){if(isSocialHref(links[j].getAttribute("href"))) return links[j];}'
        . '}'
        . 'links=qsAll("a[href]");'
        . 'for(j=0;j<links.length;j++){if(isSocialHref(links[j].getAttribute("href"))) return links[j];}'
        . 'return null;'
        ...
        . 'if(!parent) break;'
        . 'socials=qsAll("a[href]",parent).filter(function(el){return isSocialHref(el.getAttribute("href"));});'
        . 'if(socials.length>=1){'
        ...
        . 'a.innerHTML=\'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2S15.9 22 17 22s2-.9 2-2-.9-2-2-2zM7.16 14h9.69c.75 0 1.41-.41 1.75-1.03l3.58-6.49A1 1 0 0 0 21.33 5H6.21l-.94-2H1v2h2l3.6 7.59-1.35 2.44C4.52 16.37 5.48 18 7 18h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12L7.16 14z"/></svg>\';'
        . 'if((CFG.count||0)>0){'
        . 'a.innerHTML+=\'<span class="casting-main-cart-badge">\'+String(CFG.count)+\'</span>\';'
        . '}'
        . 'return a;'
        . '}'
        . 'function place(){'
        ...
        . 'if(spot.after.nextSibling){spot.parent.insertBefore(link,spot.after.nextSibling);}'
        . 'else{spot.parent.appendChild(link);}'
        ...
        . 'n++;'
        . 'if(place()||n>20){clearInterval(t);if(!document.querySelector(".casting-main-cart-social")) showFallback();}'
        . '},250);'
        . '}'
    ...
    wp_register_script('casting-main-cart-nav', false, [], '1.1', true);
    wp_enqueue_script('casting-main-cart-nav');
    wp_add_inline_script('casting-main-cart-nav', $js);
}
...
function casting_main_cart_footer_markup(): void
{
    ...
    $badge = '';
    if ($count > 0) {
        $badge = '<span class="casting-main-cart-badge">' . (int) $count . '</span>';
    }

...
if ($user && casting_is_employer_role(casting_get_user_role((int) $user->ID))) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce((string) $_POST['_wpnonce'], 'casting_login_employer')) {
        $error = 'درخواست نامعتبر است. دوباره تلاش کنید.';
    } else {
        $login = (string) ($_POST['login'] ?? '');
        $password = (string) ($_POST['password'] ?? '');
        $result = casting_login($login, $password, 'employer');
        if (!$result['ok']) {
            $error = $result['error'];
        } else {
            casting_redirect('dashboard-employer.php');
...
<main class="wrap panel-page">
  <section class="panel">
    ...
    <p class="form-foot">
      ...
      ·
      هنرجو هستید؟ <a href="login-talent.php">ورود هنرجو</a>
    </p>
  </section>
</main>

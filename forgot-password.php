...
$otp_sent = false;
$otp_verified = false;
$channel = ((string) ($_GET['channel'] ?? $_POST['channel'] ?? 'sms')) === 'email' ? 'email' : 'sms';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $channel = ((string) ($_POST['channel'] ?? 'sms')) === 'email' ? 'email' : 'sms';
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce((string) $_POST['_wpnonce'], 'casting_forgot')) {
        ...
        if ($rate_error !== null) {
            ...
            $result = casting_request_password_reset($login, 'email');
            if (!$result['ok']) {
...
if ($channel === 'sms' && !$otp_verified && $mobile !== '' && casting_otp_session_is_verified('reset', $mobile)) {
    $otp_verified = true;
    $otp_sent = true;
}
...
<main class="wrap panel-page">
  <section class="panel">
    ...
    <nav class="admin-tabs" aria-label="روش بازیابی">
      <a class="admin-tab <?= $channel === 'sms' ? 'is-active' : '' ?>" href="forgot-password.php">پیامک</a>
      <a class="admin-tab <?= $channel === 'email' ? 'is-active' : '' ?>" href="forgot-password.php?channel=email">ایمیل</a>
    </nav>

    <?php if ($channel === 'email') : ?>
      <p class="lede">نام کاربری یا ایمیل حساب را وارد کنید تا لینک تعیین رمز جدید برایتان ارسال شود.</p>
      <form class="form" method="post" action="forgot-password.php?channel=email">
        <?php wp_nonce_field('casting_forgot'); ?>
        <input type="hidden" name="channel" value="email">
        <div class="field">
          <label for="login">نام کاربری یا ایمیل</label>
          <input id="login" name="login" type="text" required autocomplete="username" value="<?= casting_e($login) ?>" placeholder="نام کاربری یا ایمیل">
        </div>
